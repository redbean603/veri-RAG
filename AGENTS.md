...
- `config.py`: Chroma host, collection names, and data paths.
...
```bash
python -c "from search import vector_search; print(vector_search('holding company stock price', query_vector=[0.0]*1536))"
...
Agent integration should call `agent_evidence_search()` when the Agent passes incoming JSON files or records, graph-selected `candidate_news_ids`, `query`, and required `query_vector`. The function validates and upserts JSON records into Chroma, runs evidence retrieval, and returns `selected_news_ids`, ranked result evidence, ingest stats, failed IDs, and ingest errors.
...
`news_id`, `summary`, and `text_vector` are required. `text_vector` is stored directly. `image_vector` is optional; when present, it is upserted into the image collection with the same `news_id`, and when absent image upsert is skipped. The Agent does not send image files. `query_vector` is required for search; this codebase does not call OpenAI or create query embeddings. `query_vector` and `text_vector` must come from the same embedding model and have the same dimension.

`vector_search()` returns formatted dictionaries with `news_id`, metadata, `content`, `summary`, cosine-derived `score`, and raw `distance`.

`evidence_search()` delegates to `hybrid_search.retrieve_and_rerank()`. It accepts optional graph-selected `candidate_news_ids`, retrieves a larger vector candidate set within that candidate set, runs keyword matching over Chroma documents and metadata, fuses candidates with RRF, and optionally reranks with `LocalReranker`.
...
When `FlagEmbedding` or the reranker model is unavailable, the reranker must return fusion/vector-score ordered results with `rank_stage="vector_fallback"` instead of failing the Agent workflow.
...
Mock Chroma and reranker calls when adding unit tests.
...
Be careful with collection resets. `load_text_collection(reset=True)` is maintenance-only and deletes and recreates the text Chroma collection. Normal Agent integration should use `agent_evidence_search()` with Agent-provided records or files.
